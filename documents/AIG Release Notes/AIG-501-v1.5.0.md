# AIG-501 Series v1.5 Release Notes
...
2. The DLM Client for the Moxa DLM Service now implements a backoff mechanism to prevent excessive message repetition during high-frequency events, optimizing events reported while ensuring critical notifications remain visible.
...
7. The refresh button of IO did not update info.
...
## Installation
Pre-request：AIG-501 Firmware Version:1.4.0 and ThingsPro Version: 2.5.0.-4404

### Upgrade Firmware

Select Download Over the Air, and input the URI link of the patch yaml. (Do not select this way, if the known issue, container IP conflicts, occurred)

(optional) Please reboot if the snapshot of the software upgrade is disabled 
Confirm that the firmware version on the Dashboard page is 1.5.0 and ThingsPro Version is  2.6.0-4499, as shown on the Dashboard page:
...
4. Confirm that the **Firmware Version** on the Dashboard page is **1.5.0** and **ThingsPro Version** is **2.6.0-4499**.

### Upgrade ThingsPro Agent
1. Deploy IoT Edge Module thingspro-agent with URI: moxa2019/thingspro-agent:2.3.0-amd64 in IoTHub.
...
1. The upgrade package size is approximately 463 MB, and the upgrade process may take 20+ minutes.
